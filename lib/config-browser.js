...
window.addEventListener('DOMContentLoaded', function handleDOMLoad () {
  ...
  [].slice.call($checkboxContainers).forEach(function bindCheckbox ($checkboxContainer) {
    ...
    if ($checkboxContainer.dataset.unsetCheckbox) {
      ...
      //Find url
      var $siteUrlContainer = document.querySelector('[data-save-siteurl]');
      var siteUrlConfigItem = $siteUrlContainer.dataset.saveSiteurl;
      var $urlinput = $siteUrlContainer.querySelector('input[type=text]');
      $urlinput.value = config[siteUrlConfigItem];

      // Add change binding for our setting
      window.addEventListener('beforeunload', function handleUrlChange (evt) {
          // Update our setting
          var result = JSON.parse(ipcRenderer.sendSync('set-config-item-sync', siteUrlConfigItem, $urlinput.value));
